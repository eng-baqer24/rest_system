  ...
  try {
    ...
    if (process.env.DATABASE_URL) {
      try {
        ...
        console.warn("Prisma read failed, using memory store:", err);
        reservations = memoryStore;
      }
    } else {
      reservations = memoryStore;
    ...
    return NextResponse.json(
      { error: "Failed to fetch reservations", reservations: memoryStore },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { date, time, guests, name, email, phone, notes, status } = body;
    ...
    if (process.env.DATABASE_URL) {
      try {
        ...
        console.warn("Prisma write error, preserved in memory:", err);
      }
    }
...
export async function PATCH(request: NextRequest) {
  try {
    const body = await request.json();
    const { id, status, notes, guests, time, date } = body;
  ...
  try {
    ...
    if (process.env.DATABASE_URL) {
      try {
        ...
        console.warn("Prisma delete warning:", err);
      }
    }
